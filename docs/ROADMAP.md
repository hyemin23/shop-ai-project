...
- [x] Supabase 브라우저 클라이언트로 `studio_history` 테이블 SELECT 성공
- [x] Supabase 서버 클라이언트(service_role)로 `studio_history` INSERT 성공
- [x] Storage 버킷에 테스트 이미지 업로드/다운로드 성공
- [x] RLS 정책 동작 확인: session_id 기반 접근 제어
...
- [x] Flash 모델로 의류 교체 이미지 생성 성공
- [x] Pro 모델로 의류 교체 이미지 생성 성공
- [x] Pro 모델 실패 시 Flash Fallback 동작 확인
- [x] 잘못된 이미지 입력 시 구조화된 에러 반환
- [x] 3초 쿨다운 디바운싱 동작 확인
...
- [x] 10MB 이하 JPG/PNG/WebP 이미지 업로드 성공
- [x] 10MB 초과 이미지 업로드 시 에러 반환
- [x] 지원하지 않는 포맷 업로드 시 에러 반환
- [x] 업로드된 이미지 URL로 접근 가능 확인
- [x] 썸네일 (200px) 자동 생성 확인
- [x] 세션 ID 쿠키 생성/유지 확인
...
- [x] `/api/studio/try-on` POST: Base + Reference 이미지 -> 결과 이미지 URL 반환
- [x] `/api/studio/color-swap` POST: 원본 이미지 + HEX 색상 -> 결과 이미지 URL 반환
- [x] `/api/studio/pose-transfer` POST: 원본 이미지 + 프리셋 ID -> 결과 이미지 URL 반환
- [x] `/api/history` GET: 세션별 히스토리 목록 반환 (최신순)
- [x] `/api/history` DELETE: 특정 히스토리 삭제 성공
- [x] 잘못된 입력 시 적절한 에러 코드 + 메시지 반환
- [x] 히스토리에 모델 정보, Fallback 여부 기록 확인
...
- [x] 의류 교체: 이미지 업로드 -> 생성 -> 결과 표시 -> 다운로드 전체 플로우
- [x] 색상 변경: 이미지 업로드 -> 색상 선택 -> 생성 -> 결과 표시 전체 플로우
- [x] 포즈 변경: 이미지 업로드 -> 프리셋 선택 -> 생성 -> 결과 표시 전체 플로우
- [x] 히스토리: 실제 생성 내역이 히스토리에 표시되는지 확인
- [x] 재생성 버튼 동작 확인
...
- [x] 10MB 초과 파일 업로드 시 토스트 알림 표시
- [x] API 타임아웃 시 재시도 버튼 표시 + 자동 재시도 동작
- [x] Rate Limit 초과 시 대기 안내 메시지 표시
- [x] 의류 영역 감지 실패 시 적절한 안내 메시지
- [x] 고품질 -> Flash Fallback 시 토스트 알림 확인
- [x] 오프라인 상태에서 생성 버튼 비활성화 확인
- [x] 연속 클릭 시 3초 쿨다운 동작 확인
...
- [x] 카카오 로그인 -> 프로필 자동 생성 확인
- [x] 구글 로그인 -> 프로필 자동 생성 확인
- [x] 로그인 후 기존 세션 히스토리가 user_id에 연결되는지 확인
- [x] 로그아웃 -> 재로그인 시 세션 유지 확인
- [x] 비로그인 사용자도 스튜디오 이용 가능 확인
...
- [x] 토큰 패키지 선택 -> 토스 결제 -> 토큰 충전 전체 플로우
- [x] 결제 취소 시 토큰 미충전 확인
- [x] 이미지 생성 시 토큰 차감 확인 (모드/해상도별 정확한 양)
- [x] 토큰 부족 시 충전 유도 모달 표시
- [x] Fallback 발생 시 토큰 차액 환불 확인
- [x] 토스 웹훅으로 결제 상태 동기화 확인
