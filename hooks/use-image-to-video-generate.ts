...
interface UseImageToVideoGenerateReturn {
  status: VideoGenerationStatus;
  videoUrl: string | null;
  error: string | null;
  elapsedSeconds: number;
  isOnline: boolean;
  submit: (params: ImageToVideoRequest) => Promise<void>;
  reset: () => void;
}
...
export function useImageToVideoGenerate({
  ...
  onError,
  onTokenInsufficient,
}: UseImageToVideoGenerateOptions = {}): UseImageToVideoGenerateReturn {
  ...
  const pollTask = useCallback(
    (taskId: string) => {
      ...
      pollingRef.current = setInterval(async () => {
        ...
        try {
          const res = await fetch(`/api/video/image-to-video/${taskId}`);
  ...
  const submit = useCallback(
    async (params: ImageToVideoRequest) => {
      if (!navigator.onLine) {
