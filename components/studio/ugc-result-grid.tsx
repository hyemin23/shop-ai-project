  ...
  if (items.length === 0) {
    return (
      <div className="flex h-48 items-center justify-center rounded-lg border border-dashed text-sm text-muted-foreground">
        의류 이미지를 업로드하고 장소를 선택한 뒤 생성 버튼을 눌러주세요.
      </div>
    );
  }
    ...
    <div className="space-y-3">
      {successItems.length > 0 && (
        <div className="flex items-center justify-between">
          <p className="text-sm text-muted-foreground">
            {successItems.length}/{items.length}개 UGC 이미지 생성 완료
          </p>
          <Button
            variant="outline"
            size="sm"
      ...
      <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
        {items.map((item) => (
          <div
            key={item.index}
            className="group relative aspect-[9/16] overflow-hidden rounded-md border"
          >
              ...
              <>
                ...
                <div className="absolute right-1 bottom-1 opacity-0 transition-opacity group-hover:opacity-100">
                  <Button
                    ...
                    onClick={(e) => {
                      e.stopPropagation();
                      downloadImage(item.resultImageUrl!, `ugc_${item.index + 1}.webp`);
                    }}
                  >
