...
1. **Next.js 16 App Router 아키텍처**
   ...
   - **🔄 NEW**: Proxy (`proxy.ts`) — Middleware가 Proxy로 리네이밍 (v16.0.0)
...
2. **Supabase 통합 패턴**
   - 세 가지 클라이언트 타입의 정확한 사용:
     ...
     * Proxy: `@/lib/supabase/proxy`의 `updateSession()` (Next.js 16에서 middleware → proxy로 변경)
   - 쿠키 기반 인증 처리
   - 데이터베이스 쿼리 최적화
...
4. **인증 및 보안**
   ...
   - Proxy 기반 라우트 보호 (`proxy.ts`에서 인증 체크 및 리다이렉트)
   - 세션 관리 및 갱신
...
#### 1. Proxy (proxy.ts) — Middleware 대체

Next.js 16부터 `middleware.ts`가 `proxy.ts`로 리네이밍되었습니다. 기능은 동일하지만 파일명과 export 함수명이 변경됩니다.

```typescript
// ✅ Next.js 16: proxy.ts (프로젝트 루트 또는 src/ 디렉토리)
import { NextResponse } from 'next/server'
import type { NextRequest } from 'next/server'

// named export 또는 default export 모두 가능
...
// ❌ 금지: middleware.ts 사용 (deprecated)
// middleware.ts → proxy.ts로 마이그레이션 필요
// 마이그레이션 명령: npx @next/codemod@canary middleware-to-proxy .
```

**Proxy 주요 특징:**
- 요청 완료 전에 서버에서 실행되며 rewrite, redirect, 헤더/쿠키 수정 가능
- `matcher` 설정으로 특정 경로만 타겟팅
- 느린 데이터 페칭용이 아님 — 권한 기반 리다이렉트 같은 낙관적 체크에 사용
- `fetch`의 `options.cache`, `options.next.revalidate`, `options.next.tags`는 Proxy 내에서 효과 없음
- Node.js 런타임 기본 사용

#### 2. async request APIs 처리
```typescript
...
2. **성능 최적화**: Next.js 16 새 기능(Streaming, after API, Proxy 등) 적극 활용
